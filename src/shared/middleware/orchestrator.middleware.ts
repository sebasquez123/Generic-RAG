...
export class AuthMiddleware implements NestMiddleware {
  async use(
    ...
    if (!agentTokenId) {
      logger.warn(
        'No authorization token provided, continuing as public request',
      );
      return next();
    }
    ...
    const getBotInfo: BotEntity = await getBotInfoRepository(
      parsedToken.bot_id,
    );
    if (!getBotInfo?.bot || !getBotInfo?.contract)
      throw new ForbiddenException('Bot not found for the provided token');

    if (getBotInfo.bot.suspended)
      throw new ForbiddenException('Bot is suspended');
